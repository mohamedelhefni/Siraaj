...
func TestGetTopProperties(t *testing.T) {
	tests := []struct {
		name           string
		queryParams    string
		setupMock      func(*mocks.MockEventService)
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockEventService) {
				m.EXPECT().
					GetTopProperties(gomock.Any(), gomock.Any(), 20, gomock.Any()).
					Return([]map[string]interface{}{
						{"key": "browser", "value": "Chrome", "count": 100},
					}, nil).
					Times(1)
			},
			expectedStatus: http.StatusOK,
		...
		{
			name:        "With filters",
			queryParams: "?project=myapp&limit=10",
			setupMock: func(m *mocks.MockEventService) {
				m.EXPECT().
					GetTopProperties(gomock.Any(), gomock.Any(), 10, gomock.Any()).
		...
		},
		{
			...
			setupMock: func(m *mocks.MockEventService) {
				m.EXPECT().
					GetTopProperties(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
					Return(nil, errors.New("error")).
					Times(1)
			},
			expectedStatus: http.StatusInternalServerError,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/properties"+tt.queryParams, nil)
			w := httptest.NewRecorder()

			handler.GetTopProperties(w, req)
